...
class NEEMInterface:
    ...
    def add_vr_subaction_with_task(self, parent_action_iri, sub_action_type,
        ...
        for object_with_class_name in objects_with_class_name:
            ...
            if len(objects) > 1:
                ...
                somifiedIndividualName = "soma:'" + objects[
                    1] + "_1'"  # add _1 here as instance name so that it is different from class name and matches to owl NamedIndividual name
    ...
    def define_task(self, task_def, environment, agent):
        ...
        task_def_query_response = self.prolog.ensure_once(f"""
                    ...
                    """)
        if task_type != "":
            task_type_query_response = self.add_task_type(task_type, action_type, environment, environment_type, agent,
                                                          agent_type)
            ...
            dest_query_response = ""
            if data["target_object"] != "":
                dest_query_response = self.add_task_destination_obj(task_type_query_response, dest_obj, dest_obj_role)

            # here only add substance if it is provided as
            substance_query_response = ""
            if data["substance"] != "":
                substance_query_response = self.add_task_substance(task_type_query_response, substance, substance_role)
